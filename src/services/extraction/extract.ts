...
9. DOPPIA COLONNA Kg/T (SOLO per metriche GHG): Se una tabella ha ENTRAMBE le colonne "Kg CO2eq" (o "kg CO2e") E "T CO2eq" (o "t CO2e", "tonnellate CO2e"), estrai SEMPRE il valore dalla colonna TONNELLATE (T CO2eq). NON usare la colonna Kg. Questo vale anche se i valori in Kg sono piu grandi — la colonna T e quella corretta per tCO2e.
10. SCOPE COMBINATI in highlights/card: Se il testo riporta un valore con etichetta del tipo "Scope 1+2", "Scope 1 e 2", "Emissioni Scope 1+2 MB", "Totale Scope 1+2+3" o simile, NON assegnare quel valore a scope_1 né a scope_2 individualmente. Il valore e un aggregato e non puo essere attribuito a un singolo scope. In questo caso, restituisci candidates vuoto per questa metrica.
11. SCOPE_2 MULTI-LIVELLO TENSIONE: Alcune tabelle di scope_2 riportano righe separate per "BT" (bassa tensione), "MT" (media tensione), "AT" (alta tensione) o "Elettricita BT", "Elettricita MT". Per scope_2_location_based e scope_2_market_based, estrai il TOTALE che include TUTTI i livelli di tensione, NON solo la prima riga (BT). Se non esiste una riga TOTALE esplicita, NON sommare le righe — restituisci found=false.
12. SCOPE_3 TOTALE vs UPSTREAM: Alcuni report mostrano un diagramma o tabella che elenca solo le categorie UPSTREAM (acquisti, trasporti in entrata, rifiuti, ecc.) con un subtotale (es. 49.290 tCO2e), mentre il totale scope_3 VERO include anche le categorie DOWNSTREAM (uso del prodotto, fine vita, ecc.). Per scope_3_total_ghg_emissions, cerca sempre il TOTALE COMPLESSIVO che include sia upstream sia downstream. Se il testo riporta un totale piu grande del subtotale upstream, il totale piu grande e quello corretto.
13. CONVERSIONE GJ → MWh: Se il valore di energia e espresso in GJ e devi normalizzarlo in MWh, usa: MWh = GJ × 0.2778 (equivalente a GJ ÷ 3.6). NON usare GJ ÷ 3600 (che darebbe GWh, 1000 volte piu piccolo). Esempio: 74.014 GJ = 20.559 MWh (NON 20,56 MWh che implica GWh).
